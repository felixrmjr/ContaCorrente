...
namespace WR.Modelo.Api.Filters
{
    public class DomainExceptionFilter : ExceptionFilterAttribute
    {
        ...
        public DomainExceptionFilter(IStringLocalizer localizer)
        {
        ...
        protected virtual JsonResult Exception(Exception exception)
        {
            ...
            var error = new Error(exception.GetType().FullName, exception.GetFullMessage());
        ...
        {
            return string.IsNullOrEmpty(_localizer[exception.Message]) ? exception.Message : _localizer[exception.Message, ObterArgumentos(exception.Arguments)];
        }
        ...
        private object[] ObterArgumentos(object[] argumentos)
        {
            ...
            {
                return string.IsNullOrWhiteSpace(_localizer[arg.ToString()]) ? arg.ToString() : _localizer[arg.ToString()];
            }).ToArray();
        }
